...
export const api = {
  ...
  ecommerce: {
    create: (
      ...
      formData.append("storeTypeId", storeTypeId.toString());
      ...
      return request<any>("/ecommerce/admin/saveStore", {
        method: "POST",
        body: formData,
      });
    },
  ...
  media: {
    uploadImage: async (file: File): Promise<string> => {
      try {
        ...
        // Find base url dynamically or fall back to localhost:8080
        const baseUrl = typeof window !== 'undefined' && (window as any).API_BASE_URL 
        ...
        const response = await fetch(`${baseUrl}/api/ecommerce/products/upload-image`, {
          method: 'POST',
          body: formData,
          headers
        });
        ...
        console.warn('Backend upload failed/offline, fallback to local FileReader data URI:', err);
