...
// Base-URL resolution now lives in one place: @/lib/config/apiConfig.
// (Previously hard-coded inline here, coupling config to transport.)
...
export async function request<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  try {
    if (isProjectApiPath(path) && typeof window !== 'undefined') {
      // A single published-project read (GET /projects/:id) is PUBLIC — it powers
      // the generated storefront, login and dashboard for customers who don't
      // hold the owner's authToken. Only the projects list and mutations require
      // an owner token.
      const method = (options?.method || 'GET').toUpperCase();
      const isPublicProjectRead = method === 'GET' && /^\/projects\/\d+/.test(path);
      const token = localStorage.getItem('authToken');
      if (!token && !isPublicProjectRead) {
        throw new Error('API Error: 401 Unauthorized - Authentication token required');
      }
    }
  ...
  headers["Content-Type"] = "application/json";
    ...
    if (err?.message?.startsWith('API Error: 401') || err?.message?.startsWith('API Error: 403')) {
      if (typeof window !== 'undefined') {
        localStorage.removeItem('authToken');
      }
    }
    // Catch network-level errors (e.g. backend server is offline), API 401/403/404s, or API 500s (e.g. database/JPA transactional failures)
    const isNetworkError = !err?.message?.startsWith('API Error:');
    const isFallbackStatus = err?.message?.startsWith('API Error: 401') || 
                             err?.message?.startsWith('API Error: 403') ||
                             err?.message?.startsWith('API Error: 404') || 
    ...
    if (isNetworkError || isFallbackStatus) {
      if (isProjectApiPath(path)) {
        ...
        if (path.match(/\/projects\/\d+/) && method === 'GET') {
          ...
          // Backend unreachable AND not cached offline. Return null instead of
          // throwing so callers can show a friendly "backend offline" state
          // rather than crashing with a dev error overlay.
          console.warn(`[API Fallback Warning] Project with ID ${id} not found in offline storage (backend unreachable).`);
