...
import { MEDICAL_NICHES } from '@/components/preview/medical/medicalNiches';
...
export default function MedicalShopSelectorModal({
  ...
  const [phase, setPhase] = useState<'niche' | 'form'>('niche');
  const [selectedNiche, setSelectedNiche] = useState<string>(MEDICAL_NICHES[0].id);
  ...
  const handleGenerate = () => {
    const configData = {
      selectedNiche,
      nicheName: MEDICAL_NICHES.find(n => n.id === selectedNiche)?.name || 'Retail Pharmacy',
    ...
    <div className="fixed inset-0 bg-slate-950/50 backdrop-blur-md z-50 flex items-center justify-center p-4 md:p-6 animate-fade-in text-slate-800">
      ...
      >
        ...
        <div className="p-6 md:p-8 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500 font-medium">
              {phase === 'niche' ? (
                'First — choose your pharmacy template'
              ) : (
                <>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 bg-slate-50/20">
          ...
          {/* PHASE: NICHE TEMPLATE SELECTION (shown first) */}
          {phase === 'niche' && (
            <div className="space-y-6">
              <div className="text-center max-w-lg mx-auto">
                <h3 className="text-sm font-extrabold text-slate-800">Which kind of pharmacy are you building?</h3>
                <p className="text-[11px] text-slate-500 mt-1">Each template has its own look, landing page, and login screen. You can fine-tune colours next.</p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {MEDICAL_NICHES.map((niche) => (
                  <button
                    key={niche.id}
                    onClick={() => setSelectedNiche(niche.id)}
                    className={`group flex flex-col overflow-hidden rounded-2xl border text-left shadow-sm transition cursor-pointer relative ${
                  ...
                  >
                    <div className="relative h-28 w-full overflow-hidden">
                      <img src={niche.image} alt={niche.name} loading="lazy" className="h-full w-full object-cover transition duration-500 group-hover:scale-110" />
                      <div className={`absolute inset-0 bg-gradient-to-t ${niche.gradient} opacity-40 mix-blend-multiply`} />
                      <span className="absolute left-2.5 top-2.5 rounded-full bg-white/90 px-2 py-0.5 text-[9px] font-black uppercase tracking-wide text-slate-800">
                        {niche.emoji} {niche.vibe}
                      </span>
                    </div>
                    <div className="p-4">
                      <h4 className="font-extrabold text-slate-800 text-sm">{niche.name}</h4>
                      <p className="text-[10px] text-slate-400 font-semibold leading-normal line-clamp-2 mt-1">{niche.desc}</p>
                    </div>
                    {selectedNiche === niche.id && (
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-between items-center">
          <button
            onClick={phase === 'niche' ? onClose : (step === 1 ? () => setPhase('niche') : handleBack)}
          ...
          >
            {phase === 'niche' ? 'Cancel' : '➔ Back'}
          </button>

          <div className="flex items-center gap-3">
            <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">
              {phase === 'niche' ? 'Choose template' : `STEP ${step} / 5`}
            </span>
            ...
            {phase === 'niche' ? (
              ...
              >
                Continue with {MEDICAL_NICHES.find(n => n.id === selectedNiche)?.name} ➔
              </button>
            ) : step < 5 ? (
              <button
                onClick={handleNext}
