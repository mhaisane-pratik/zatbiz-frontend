...
export default function ProjectsGrid({ projects, onDeleteProject, onNavigateToTemplates }: ProjectsGridProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Stores created summary */}
      <div className="flex items-center gap-3">
        <span className="inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-[#eae8ff] text-[#5c3bee] text-xs font-black">
          <i className="fa-solid fa-store" />
          {mergedWorkspaces.length} {mergedWorkspaces.length === 1 ? 'website created' : 'websites created'}
        </span>
        <span className="text-[11px] font-semibold text-slate-400">
          Click a store name or “Admin Panel” to manage its products & orders.
        </span>
      </div>
              ...
              <div className="p-5 flex-1 flex flex-col justify-between relative bg-white">
                <div className="flex gap-3.5 items-start">
                  ...
                  <div className="min-w-0">
                    <Link
                      href={w.isMock ? '#' : `/preview/${w.id}/dashboard?admin=1`}
                      title="Open admin panel"
                      className="font-extrabold text-slate-900 hover:text-[#5c3bee] text-xs truncate leading-tight uppercase tracking-wider block"
                    >
                      {w.name}
                    </Link>
                ...
                <div className="flex items-center justify-between text-[10px] text-slate-450 font-bold uppercase tracking-wider">
                  ...
                  <div className="flex items-center gap-2 relative">
                    <Link
                      href={`/preview/${w.id}/dashboard?admin=1`}
                      className="px-3.5 py-2 bg-[#5c3bee] hover:bg-[#4f46e5] text-white rounded-lg text-[9px] font-black tracking-wider uppercase transition"
                    >
                      Admin Panel
                    </Link>
                    <Link
          ...
          {paginated.map((w) => (
            ...
            >
              ...
              <div className="flex items-center gap-3.5 flex-shrink-0">
                ...
                <Link
                  href={`/preview/${w.id}/dashboard?admin=1`}
                  className="px-3.5 py-2 bg-[#5c3bee] hover:bg-[#4f46e5] text-white rounded-xl text-[10px] font-black uppercase tracking-wider transition"
                >
                  Admin Panel
                </Link>
                <Link
                ...
                <Link
                  href={`/builder/${w.id}`}
                  className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] font-black uppercase tracking-wider rounded-xl shadow transition"
                >
                  Edit
                </Link>
              </div>
