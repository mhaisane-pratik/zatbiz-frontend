...
import GymFullPreview from '@/components/preview/gym/GymFullPreview';
...
export default function GymSelectorModal({
  ...
  const [fullPreviewTheme, setFullPreviewTheme] = useState<any>(null);
  // Until the user picks a header image, each theme shows its own hero art.
  const [bgChosen, setBgChosen] = useState(false);

  // Commits exactly what the preview is showing.
  const commitSelection = (theme: any) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-md z-50 flex items-center justify-center p-4 md:p-6 animate-fade-in">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar bg-slate-50/20">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                ...
                <div className="flex flex-col bg-gradient-to-br from-indigo-50/50 via-white to-purple-50/30 border border-dashed border-indigo-300 rounded-2xl overflow-hidden hover:border-indigo-500 hover:shadow-xl transition-all duration-300 group">
                  <div className="w-full h-32 bg-slate-50/85 flex flex-col items-center justify-center relative flex-shrink-0 border-b border-slate-100">
                    ...
                    <span className="absolute top-2.5 left-2.5 bg-indigo-50 border border-indigo-200 text-indigo-600 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-wider">
                      Blank Slate
                    </span>
                  </div>
                ...
                {filteredCategories.map((cat) => (
                  ...
                  >
                    <div className="w-full h-32 overflow-hidden relative flex-shrink-0 border-b border-slate-100">
                      ...
                      {/* Floating emoji icon */}
                      <div className="w-9 h-9 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-base absolute bottom-2 right-2.5 z-10 shadow-md">
                        {cat.icon}
                      </div>
                    </div>
                    <div className="p-4 flex-1 flex flex-col justify-between text-left font-sans">
                      ...
                      <button
                        ...
                        className="w-full py-2.5 text-[10px] font-black bg-slate-50 border border-slate-200/80 hover:bg-indigo-600 hover:border-indigo-600 hover:text-white text-slate-600 rounded-xl transition uppercase tracking-wider flex items-center justify-center gap-1 cursor-pointer"
                      >
                        Select & View Themes ➔
                      </button>
                    </div>
          ...
          {wizardStep === 'theme' && (
            ...
            <div className="space-y-6">
              <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-2xl text-left">
                <p className="text-[11px] text-indigo-800 font-bold leading-normal">
                    ...
                    >
                      ...
                      <div className={`w-full h-28 relative flex-shrink-0 border-b border-slate-100 bg-gradient-to-br ${theme.gradient} flex items-center justify-center`}>
                        ...
                        <span className={`absolute top-3 right-3 text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full ${
                          isDarkTheme ? 'bg-zinc-950 text-zinc-300 border border-zinc-800' : 'bg-white text-slate-700 border border-slate-200'
                        }`}>
                        ...
                        <div className="space-y-2">
                          <button
                            onClick={() => {
                              ...
                              setFullPreviewTheme(theme);
                            }}
                            className="w-full py-2.5 text-[10px] font-black text-white rounded-xl transition uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer border-none hover:brightness-110"
                            style={{ backgroundColor: theme.primaryColor }}
                          >
                          ...
                          <button
                            ...
                            className="w-full py-2.5 text-[10px] font-black bg-slate-50 border border-slate-200/80 hover:bg-indigo-600 hover:border-indigo-600 hover:text-white text-slate-600 rounded-xl transition uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer"
                          >
          ...
          {wizardStep === 'bg-image' && (
            ...
            <div className="space-y-6">
              <div className="bg-indigo-50 border border-indigo-200 p-5 rounded-2xl text-left flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs rounded-xl shadow-md transition cursor-pointer border-none flex-shrink-0"
                >
                ...
                {GYM_BG_IMAGES.map((img) => {
                  ...
                  return (
                    <button
                      key={img.id}
                      onClick={() => {
                        setSelectedBg(img.url);
                        setBgChosen(true);
                      }}
                      className={`flex flex-col bg-white border rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 text-left ${
                        isSelected ? 'border-indigo-600 ring-2 ring-indigo-600/35 scale-[1.02] shadow-md' : 'border-slate-200 hover:border-slate-300 hover:scale-[1.01]'
                      }`}
                    >
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-start gap-4 p-5 bg-white border rounded-2xl cursor-pointer text-left transition-all duration-300 ${
                        isSelected ? 'border-indigo-600 ring-2 ring-indigo-600/35 scale-[1.01] shadow-md' : 'border-slate-200 hover:border-slate-300'
                      }`}
                    >
            ...
            <div className="space-y-6 text-left">
              ...
              <div className="bg-slate-50 border border-slate-200 p-5 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex gap-2 flex-shrink-0">
                  <button
                    onClick={() => setFullPreviewTheme(selectedTheme)}
                    className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-black text-xs rounded-xl border border-slate-200 transition cursor-pointer flex items-center gap-1.5"
                  >
                    👁 Open Full Preview
                  </button>
                  <button
                    onClick={() => commitSelection(selectedTheme)}
                    className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs rounded-xl shadow-md transition cursor-pointer border-none flex items-center gap-1.5"
                  >
                ...
                <div className="font-sans text-slate-800">
                  ...
                  <header className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white">
                    ...
                    <nav className="hidden sm:flex gap-6 text-[10px] uppercase font-black text-slate-500 tracking-wider">
                      <span className="hover:text-slate-800 cursor-pointer">Classes</span>
                      <span className="hover:text-slate-800 cursor-pointer">Schedule</span>
                      <span className="hover:text-slate-800 cursor-pointer">Coaches</span>
        ...
        {/* Modal Footer */}
        <div className="p-4 bg-slate-50 border-t border-slate-200 text-center text-[10px] text-slate-400 font-bold uppercase tracking-wider">
          {wizardStep === 'niche'
      ...
      {/* FULL INTERACTIVE THEME PREVIEW (landing → login → member portal) */}
      {fullPreviewTheme && (
        <GymFullPreview
          theme={fullPreviewTheme}
          category={selectedCategory || 'Gym'}
          bgImage={selectedBg}
          bgChosen={bgChosen}
          loginLayout={selectedLogin}
          onClose={() => setFullPreviewTheme(null)}
          onUseTheme={() => {
            setFullPreviewTheme(null);
            commitSelection(fullPreviewTheme);
          }}
        />
      )}
    </div>
