...
export default function RealEstateLanding({
  ...
  handleAddToCart,
  gymInfo,
}: RealEstateLandingProps) {
  return (
    <main className="divide-y divide-slate-100 bg-white">
      {currentPageBlocks.length === 0 ? (
        <div className="text-center py-32 text-slate-400">
          <span className="text-4xl block mb-4">🏠</span>
          <h2 className="text-lg font-bold text-slate-900 mb-1">Realty Listing Site</h2>
          <p className="text-xs max-w-xs mx-auto">
            Open the dashboard editor and construct property page layouts.
          </p>
        </div>
      ) : (
