...
export default function MedicalShopLogin({
  projectId,
  isSignUp,
  setIsSignUp,
  companyName,
  ...
  successMessage,
  handleLoginSubmit,
}: MedicalShopLoginProps) {
  const illustrationUrl = 'https://images.unsplash.com/photo-1586015555751-63bb77f4322a?w=800&auto=format&fit=crop&q=80';

  return (
    <main className="min-h-screen bg-slate-50 flex items-stretch text-slate-800 font-sans">
      {/* Left side: Pharmacy Illustration */}
      <div
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xl font-black uppercase tracking-wider">{companyName || 'City Pharmacy'}</span>
        </div>
        ...
        <div className="space-y-4 max-w-md text-left my-auto">
          <span className="bg-teal-500/25 border border-teal-400/20 px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider">
            Pharmacy Gateway Portal
          </span>
          <h2 className="text-3xl font-black tracking-tight leading-tight">
            Your Health & Wellness, Delivered
          </h2>
          <p className="text-xs text-slate-350 font-medium leading-relaxed">
            Sign in to upload prescriptions, browse wellness/medical catalog categories, order medications with flat coupons, and review transaction ledgers.
          </p>
        </div>

        <p className="text-[10px] text-teal-400 font-black uppercase tracking-wider">
          © 2026 {companyName || 'City Pharmacy'}. Health Systems.
        </p>
      </div>
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="text-left">
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">
              {isSignUp ? 'Create Client Account' : 'Sign In to Pharmacy'}
            </h2>
            <p className="text-xs text-slate-400 font-semibold mt-1">
              {isSignUp ? 'Register to place digital prescription orders.' : 'Sign in with credentials to access portal.'}
            </p>
          </div>
          ...
          {!isSignUp && (
            <div className="rounded-2xl border border-teal-100 bg-teal-50/20 p-4 text-left">
              <p className="text-[9px] font-black uppercase tracking-wider text-teal-650">
                Demo Accounts
              </p>
              <div className="mt-2 space-y-1 text-xs text-slate-600">
                <p>
                  <span className="font-bold text-slate-800">Admin Email:</span>{' '}
                  <span className="bg-white border border-teal-100 rounded px-1.5 py-0.5 font-mono text-[10px] text-teal-650 font-bold">admin@gmail.com</span>
                </p>
                <p>
                  <span className="font-bold text-slate-800">Customer Email:</span>{' '}
                  <span className="bg-white border border-teal-100 rounded px-1.5 py-0.5 font-mono text-[10px] text-teal-650 font-bold">customer@gmail.com</span>
                </p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full mt-4 rounded-xl py-3 px-4 text-xs font-bold text-white bg-teal-600 hover:bg-teal-750 cursor-pointer shadow-md shadow-teal-500/15 uppercase tracking-wider transition-all"
            >
              {isSignUp ? 'Create Client Account' : 'Authenticate Credentials'}
            </button>
          </form>
          ...
          <div className="flex items-center justify-between text-xs pt-4 border-t border-slate-100">
            ...
            >
              {isSignUp ? 'Already registered? Sign In' : 'New client? Register'}
            </button>
            <Link
