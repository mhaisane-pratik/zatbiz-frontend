...
export function CategoryLoginTemplate({
  ...
  onEdit,
  onPickFile,
}: LoginTemplateProps & LoginEditProps) {
  const router = useRouter();
  const [slideIndex, setSlideIndex] = React.useState(0);

  // Default slides, overridable from Theme Studio Layout editor (restaurantInfo.layoutContent.login.slide1..3)
  const defaultSlides = [
  ...
  const loginLc = restaurantInfo?.layoutContent?.login || {};
  const loyaltySlides = defaultSlides.map((slide, i) => {
    const custom = loginLc[`slide${i + 1}`] || {};
    return {
      title: custom.title || slide.title,
      highlight: custom.highlight || slide.highlight,
      desc: custom.desc || slide.desc
    };
  });

  React.useEffect(() => {
    // Freeze the carousel while editing so text doesn't move under the caret.
    if (editMode) return;
  ...
  return (
    <main className={`min-h-screen flex items-center justify-center p-0 md:p-6 ${mainBgClass}`}>
      {editMode && <style>{EDIT_STYLES}</style>}
      ...
      <div className={cardClass}>
        ...
        {!isMinimal && (
          <div data-section="panel" className={`hidden md:flex md:col-span-5 bg-neutral-950 text-white p-10 flex-col justify-between relative overflow-hidden ${editMode ? 'zb-img-wrap' : ''} ${
            ...
            <div className="space-y-6 z-10 text-left my-auto transition-all duration-500">
              <h2 className="text-3xl font-black uppercase tracking-tight text-white leading-tight font-serif min-h-[72px]">
                <Ed path={`slide${slideNo}.title`} value={loyaltySlides[slideIndex].title} edit={editMode} onEdit={onEdit} />
                {' '}<br/>
                <Ed path={`slide${slideNo}.highlight`} value={loyaltySlides[slideIndex].highlight} edit={editMode} onEdit={onEdit} as="span" style={{ color: themeColor }} />
              </h2>
              <Ed
                path={`slide${slideNo}.desc`}
                value={loyaltySlides[slideIndex].desc}
                edit={editMode}
                onEdit={onEdit}
                as="p"
                className="block text-slate-300 text-xs font-semibold leading-relaxed min-h-[48px]"
                multiline
              />
              ...
              {editMode && (
                <div className="flex items-center gap-2 pt-2">
                  {loyaltySlides.map((_, i) => (
                    <button
                      key={i}
                      type="button"
                      onClick={() => setSlideIndex(i)}
                      title={`Edit slide ${i + 1}`}
                      className="w-6 h-6 rounded-full text-[10px] font-black border cursor-pointer transition"
                      style={
                        slideIndex === i
                  ...
                  <span className="text-[9px] text-white/50 font-bold uppercase tracking-wider">← click to edit each slide</span>
                </div>
              )}
            </div>
