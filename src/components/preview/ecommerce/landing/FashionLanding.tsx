...
export default function FashionLanding({
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans ${isLight ? 'bg-[#FAF9F6] text-stone-900' : 'bg-stone-955 text-stone-100'}`}>
      {/* Announcement Bar */}
      ...
      {isDetailModalOpen && selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="w-full max-w-2xl bg-stone-900 border border-white/10 rounded-3xl p-6 relative flex flex-col md:flex-row gap-6 my-8 text-white shadow-2xl">
            ...
            <div className="w-full md:w-1/2 aspect-square md:aspect-[3/4] overflow-hidden bg-stone-955 rounded-2xl border border-white/5">
            ...
            <div className="flex-1 flex flex-col justify-between space-y-4">
              <div className="space-y-3">
                ...
                <p className="text-[10px] text-slate-405 leading-relaxed">{parseDesc(selectedProduct.description).text}</p>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px] font-bold">
                      {Object.entries(parseDesc(selectedProduct.description).nicheFields).map(([k, v]: any) => (
                        <div key={k} className="border-b border-white/5 pb-1">
                          <span className="text-slate-455 block capitalize text-[8px]">{k.replace(/([A-Z])/g, ' $1')}:</span>
