...
export default function GroceryLanding({
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-slate-900 text-slate-100">
          ...
          <div className="pb-24 space-y-20">
              ...
              {vegetableProducts.length === 0 ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  {vegetableProducts.map((p: any) => (
                    <div key={p.id} className="group cursor-pointer bg-slate-905 border border-white/5 p-4 rounded-2xl hover:border-emerald-500 transition duration-300" onClick={() => handleOpenDetail(p)}>
                      <img src={p.imageUrl} className="w-full aspect-square object-cover rounded-xl bg-slate-955" alt="" />
                      <h5 className="text-[11px] font-bold text-white uppercase truncate mt-3">{p.name}</h5>
                      <span className="text-[10px] text-emerald-400">₹{p.price}</span>
                    </div>
            ...
            <section id="dairy" className="max-w-6xl mx-auto px-6 space-y-6">
              ...
              {dairyProducts.length === 0 ? (
                <p className="text-xs text-slate-550 italic">No dairy products cataloged yet.</p>
              ) : (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <section id="bakery" className="max-w-6xl mx-auto px-6 space-y-6">
              ...
              {bakeryProducts.length === 0 ? (
                <p className="text-xs text-slate-550 italic">No bakery products cataloged yet.</p>
              ) : (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
      ...
      {/* Footer */}
      <footer className="border-t border-slate-800 py-12 text-center text-xs text-slate-505 uppercase tracking-widest bg-slate-950">
