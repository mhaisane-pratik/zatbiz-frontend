...
export default function Landing({ projectId, projectConfig, products = [], onProductClick, cart = [], handleAddToCart, activeView, setActiveView, handleToggleWishlist, wishlist = [], handleCheckout }: any) {
  const primaryColor = projectConfig?.themeColor || '#475569';
  ...
  return (
    <div className={`min-h-screen flex flex-col ${fontClass} ${isLight ? 'bg-white text-slate-800' : 'bg-slate-955 text-white'}`}>
      ...
      <div className="py-2 text-center text-[10px] font-black tracking-widest text-white transition duration-300" style={{ backgroundColor: primaryColor }}>
        ✨ PREMIUM CUSTOM ONLINE STORE STORE - POWERED BY SHARABLE ENDPOINTS ✨
      </div>
      ...
      <header className={`sticky top-0 z-40 border-b backdrop-blur-xl ${isLight ? 'bg-white/85 border-slate-200' : 'bg-slate-900/80 border-white/5'}`}>
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => setCurrentSubPage('home')}>
            <span className="text-3xl p-1 bg-white/5 rounded-xl border border-white/10">🛍️</span>
            <div>
              <h1 className="text-sm font-black tracking-tight uppercase">{projectConfig?.projectName || 'Storefront'}</h1>
      ...
      <main className="flex-grow">
        {currentSubPage === 'home' && (
          <div className="space-y-12 pb-20">
            {/* Hero Section */}
            <section 
              className={`relative overflow-hidden py-20 px-6 text-center border-b bg-cover bg-center ${isLight ? 'border-slate-200' : 'border-white/5'}`}
              style={projectConfig?.bannerUrl ? { backgroundImage: `url(${projectConfig.bannerUrl})` } : { backgroundColor: isLight ? '#f8fafc' : '#0f172a' }}
            >
              ...
              <div className="max-w-2xl mx-auto space-y-6 relative z-10">
                ...
                <h2 className={`text-4xl md:text-5xl font-black tracking-tight leading-tight ${projectConfig?.bannerUrl ? 'text-white' : ''}`}>
                  Curated Premium Custom Online Store
                </h2>
                <p className={`text-xs font-medium leading-relaxed max-w-xl mx-auto ${projectConfig?.bannerUrl ? 'text-slate-200' : 'text-slate-400'}`}>
                  Custom Selected Products & Bespoke Design Systems. Explore items with standard design schemas configured for high-performance storefront layouts.
                </p>
                  ...
                  >
                    <div className="relative aspect-square w-full overflow-hidden rounded-xl bg-slate-900 border border-white/5 mb-3 flex items-center justify-center">
                      {product.imageUrl ? (
                        ...
                        <span className="text-4xl text-slate-655">🛍️</span>
                      )}
                      <button 
        ...
        {currentSubPage === 'about' && (
          <div className="max-w-3xl mx-auto px-6 py-16 space-y-6">
            ...
            <p className="text-xs text-slate-400 leading-relaxed font-semibold">
              Founded in 2026, our premium boutique represents the zenith of customer-driven visual layout structures. We specialize in curating top-tier Custom Online Store products directly sourced from sustainable manufacturers.
            </p>
            <p className="text-xs text-slate-400 leading-relaxed font-semibold">
          ...
          <div className="max-w-4xl mx-auto px-6 py-16 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white/5 border border-white/5 rounded-2xl p-6 space-y-4">
                ...
                <h3 className="text-sm font-black uppercase">The Future of Custom Online Store Visual Customizers</h3>
      ...
      <footer className={`border-t py-12 px-6 ${isLight ? 'bg-slate-100 border-slate-200 text-slate-600' : 'bg-slate-955 border-white/5 text-slate-400'}`}>
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 text-xs font-bold leading-relaxed">
          <div className="space-y-4">
            <h4 className="text-sm font-black uppercase text-white">Custom Online Store Hub</h4>
