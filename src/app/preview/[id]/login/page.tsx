...
import GymLogin from '@/components/preview/gym/ModernGymLogin';
import WeddingLogin from '@/components/preview/templates/wedding/Login';
import WeddingThemedLogin from '@/components/preview/templates/wedding/WeddingThemedLogin';
...
import EcommerceLogin from '@/components/preview/ecommerce/login/EcommerceLogin';
import ModernEcomLogin from '@/components/preview/ecommerce/ModernEcomLogin';
...
export default function UserWebsiteLoginPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    api.projects
      .get(projectId)
      .then((data: Project) => {
        if (!data) return; // backend unreachable / project not found
        try {
          ...
          // A saved e-commerce theme is a deliberate choice — it wins over the
          // real-estate / niche auto-detection below.
          const isThemedStore =
            config.businessType === 'ecommerce' ||
            config.businessType === 'shop' ||
            config.businessType === 'fashion' ||
            !!config.themeConfig?.category ||
            !!config.themeConfig?.suffix;
          if (isThemedStore) {
  ...
  const handleLoginSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Admin access on generated sites has been removed. Store owners/admins
    // now manage products, orders and templates exclusively from the main
    // ZATBIZ dashboard (Shopify-style). Block any legacy admin logins so no
    // admin session can ever be created on a generated website.
    const ADMIN_EMAILS = ['admin@gmail.com', 'agent@gmail.com', 'broker@gmail.com'];
    if (ADMIN_EMAILS.includes(email)) {
      setErrorMessage('Admin management has moved to the main ZATBIZ dashboard. Please manage this site from your ZATBIZ account.');
      return;
    }
  ...
  switch (templateId) {
    ...
    case 'gym':
      return <GymLogin {...loginProps} gymInfo={gymInfo} themePreset={theme} />;
    case 'wedding':
      return <WeddingThemedLogin {...loginProps} niche={companyName} />;
    case 'realestate':
    ...
    case 'storefront':
    case 'ecommerce':
      // All themed stores get the variant login. Scratch flows never reach
      // this switch (handled by the scratch branch above).
      return <ModernEcomLogin {...loginProps} />;
    default:
      return (
