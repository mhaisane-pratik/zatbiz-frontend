...
import ModernEcomStorefront, { readEcomConfig } from '@/components/preview/ecommerce/ModernEcomStorefront';
...
import RealEstateStorefront from '@/components/preview/templates/realestate/RealEstateStorefront';
import WeddingThemedStorefront from '@/components/preview/templates/wedding/WeddingStorefront';
...
function PreviewPageContent({ params }: PageProps) {
  ...
  const fetchProducts = () => {
    ...
    const isScratchEco = projectConfig?.businessType === 'ecommerce' && projectConfig?.shopNiche === 'scratch';
    const fetchFunc = isScratchEco
  ...
  useEffect(() => {
    ...
    api.gym.get(projectId)
      .then((gymData) => {
        if (gymData && gymData.projectId) {
          ...
          setProjectConfig((prev: any) => ({ ...prev, businessType: 'gym' }));
        }
      })
    ...
    api.projects
      .get(projectId)
      .then((data) => {
        if (!data) { setLoading(false); return; } // backend unreachable / not found
        setProject(data);
        try {
          ...
          // Self-heal: a store niche or theme config implies an ecommerce site
          // even when older projects never wrote businessType.
          if (!config.businessType && (config.themeConfig?.category || (config.shopNiche && config.shopNiche !== 'scratch'))) {
            config.businessType = 'ecommerce';
          }

          // Self-heal: projects generated from the storefront template carry a
          // hero with 'Shop Catalog' / 'Explore Collections' buttons — treat
          // them as ecommerce even when the config block is completely bare.
          if (!config.businessType) {
            const flatBlocks: any[] = Object.values(newPages).flat();
            const looksLikeStore = flatBlocks.some(
              (b: any) =>
                b?.type === 'hero' &&
                (b.content?.btn1Text === 'Shop Catalog' || b.content?.btn2Text === 'Explore Collections')
            );
            const storeName = `${data.name || ''} ${data.description || ''}`.toLowerCase();
            const nameLooksLikeStore =
              storeName.includes('boutique') || storeName.includes('storefront') || storeName.includes('store') || storeName.includes('shop');
            if (looksLikeStore || nameLooksLikeStore) {
              config.businessType = 'ecommerce';
            }
          }

          const isProjectGym = config.businessType === 'gym' ||
                               data.name?.toLowerCase().includes('gym') || 
                               data.name?.toLowerCase().includes('fitness') || 
                               data.name?.toLowerCase().includes('yogiraj') || 
                               data.description?.toLowerCase().includes('gym') || 
                               data.description?.toLowerCase().includes('yoga') || 
                               data.description?.toLowerCase().includes('fitness') ||
                               (parsed && Array.isArray(parsed) && parsed.some((b: any) => b.type === 'business_config' && (b.content?.businessType === 'gym' || b.content?.shopNiche === 'gym')));
          
          if (isProjectGym) {
            config.businessType = 'gym';
          }

          setPages(newPages);
          setActivePages(activePagesList);
          setProjectConfig(config);
          if (config.businessType === 'scratch') {
            api.scratch.get(projectId)
              ...
              .catch((e) => console.log('Error fetching scratch details:', e));
          } else if (config.businessType === 'restaurant' || config.businessType === 'hospital' || config.businessType === 'clinic' || config.businessType === 'school' || config.businessType === 'gym' || config.businessType === 'medical-shop' || config.businessType === 'shop' || config.businessType === 'ecommerce' || config.businessType === 'fashion') {
            setPreviewMode('template');
          }
        } catch (e) {
    ...
    // Products fetched reactively via useEffect on projectConfig
    // Fetch coupons
    ...
    if (typeof window !== 'undefined') {
      ...
      if (clientEmail && clientId) {
        const session = {
          id: (clientId && !clientId.startsWith('mock-')) ? parseInt(clientId, 10) : Math.floor(100000 + Math.random() * 900000),
  ...
  console.log(
    `--- ZATBIZ ECOM ROUTING: previewMode=${previewMode} bizType=${projectConfig?.businessType} shopNiche=${shopNiche} themeCategory=${projectConfig?.themeConfig?.category} → storefront=${
      projectConfig?.businessType === 'ecommerce' || projectConfig?.businessType === 'shop' || isFashion ? 'MODERN' : 'check-fallback'
    }`
  );
  ...
  /* ------------------------------------------------------------------
   * THEMED STORE SHORT-CIRCUIT
   * Runs before every other branch so an ecommerce project can never be
   * rendered by the generic block renderer. Uses the saved theme when the
   * project has one, otherwise resolves a variant from the niche/name.
   * ------------------------------------------------------------------ */
  {
    const allBlocksNow: any[] = Object.values(pages).flat();
    const heroNow: any = allBlocksNow.find((b: any) => b?.type === 'hero');
    const nameStr = `${project.name || ''} ${project.description || ''}`.toLowerCase();
    const nicheStr = String(shopNiche || '');

    // STRONG signals: the user explicitly created a themed e-commerce store.
    // These win over real-estate / fashion auto-detection, because a saved
    // ecom themeConfig is a deliberate choice — never a guess.
    const isDefinitelyStore =
      !isScratchEcommerce &&
      (projectConfig?.businessType === 'ecommerce' ||
        projectConfig?.businessType === 'shop' ||
        projectConfig?.businessType === 'fashion' ||
        !!projectConfig?.themeConfig?.category ||
        !!projectConfig?.themeConfig?.suffix);

    // WEAK signals: only trusted when the project isn't clearly another type.
    const isMaybeStore =
      !isScratchEcommerce &&
      !isRealEstate &&
      projectConfig?.businessType !== 'restaurant' &&
      projectConfig?.businessType !== 'gym' &&
      projectConfig?.businessType !== 'hospital' &&
      projectConfig?.businessType !== 'clinic' &&
      projectConfig?.businessType !== 'school' &&
      projectConfig?.businessType !== 'medical-shop' &&
      projectConfig?.businessType !== 'realestate' &&
      projectConfig?.businessType !== 'wedding' &&
      projectConfig?.businessType !== 'event' &&
      projectConfig?.businessType !== 'travel' &&
      ((!!nicheStr && nicheStr !== 'scratch') ||
        isFashion ||
        heroNow?.content?.btn1Text === 'Shop Catalog' ||
        heroNow?.content?.btn2Text === 'Explore Collections' ||
        nameStr.includes('boutique') ||
        nameStr.includes('storefront'));

    const isStore = isDefinitelyStore || isMaybeStore;

    if (isStore && activeView === 'landing') {
      const ecom = readEcomConfig(project);
      // Prefer the exact saved theme; then the saved category; then niche/name.
      const savedCat = projectConfig?.themeConfig?.category;
      const guess =
        savedCat ||
        (nicheStr.startsWith('fashion') || nicheStr === 'cloth'
          ? 'fashion'
          : nicheStr && nicheStr !== 'scratch'
          ? nicheStr
          : nameStr.includes('boutique') || nameStr.includes('fashion') || isFashion
          ? 'fashion'
          : 'marketplace');
      const resolved = ecom.theme || getEcomThemes(guess)[0];

      if (resolved) {
        return (
          <ModernEcomStorefront
            theme={resolved}
            brand={ecom.brand || project.name.replace(' Site', '')}
            logoIcon={ecom.logoIcon}
            projectId={projectId}
            overrides={ecom.overrides}
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 blueprint-grid relative flex flex-col justify-between overflow-x-clip">
      ...
      <div className="flex-1 pb-24 relative z-10">
        ...
        {activeView === 'landing' && (
          (() => {
            ...
            if (previewMode === 'template' && !isScratchEcommerce) {
              ...
              if (isWedding) {
                return (
                  <WeddingThemedStorefront
                    projectId={projectId}
                    project={project!}
                    currentPageBlocks={currentPageBlocks}
                    dbProducts={dbProducts}
                    customerSession={customerSession}
                    niche={projectConfig?.weddingCategory || projectConfig?.businessType}
                    openProductDetail={openProductDetail}
                    handleAddToCart={(p) => handleAddToCart(p, 'M', 'Black', 1)}
                    onLogout={handleLogout}
                  />
                );
              }
              ...
              if (projectConfig?.businessType === 'ecommerce' || projectConfig?.businessType === 'shop' || isFashion) {
                // Themed stores render the modern storefront — the exact
                // component previewed in the wizard and edited in the studio.
                // An ecommerce project must NEVER fall back to the legacy page,
                // so resolution cascades: saved themeConfig → shopNiche →
                // page-level niche detection → generic fashion/marketplace.
                const ecom = readEcomConfig(project!);
                if (!ecom.theme) {
                  const rawNiche = String(shopNiche || '');
                  const guessNiche =
                    rawNiche.startsWith('fashion') || rawNiche === 'cloth'
                      ? 'fashion'
                      : rawNiche && rawNiche !== 'scratch'
                      ? rawNiche
                      : isFashion
                      ? 'fashion'
                      : 'marketplace';
                  ecom.theme = getEcomThemes(guessNiche)[0] || null;
                }
                if (ecom.theme) {
                  return (
                    <ModernEcomStorefront
            ...
            // Last line of defence: if this project is a store by ANY signal,
            // render the modern storefront rather than raw layout blocks.
            {
              const flat: any[] = Object.values(pages).flat();
              const heroBlock: any = flat.find((b: any) => b?.type === 'hero');
              const nameStr = `${project?.name || ''} ${project?.description || ''}`.toLowerCase();
              const storeSignal =
                projectConfig?.businessType === 'ecommerce' ||
                projectConfig?.businessType === 'shop' ||
                isFashion ||
                !!projectConfig?.themeConfig?.category ||
                (!!shopNiche && shopNiche !== 'scratch') ||
                heroBlock?.content?.btn1Text === 'Shop Catalog' ||
                heroBlock?.content?.btn2Text === 'Explore Collections' ||
                nameStr.includes('boutique') ||
                nameStr.includes('storefront');

              if (storeSignal && !isScratchEcommerce) {
                const ecom = readEcomConfig(project!);
                const rawNiche = String(shopNiche || '');
                const guess =
                  rawNiche.startsWith('fashion') || rawNiche === 'cloth'
                ...
                if (resolved) {
                  return (
                    <ModernEcomStorefront
                      theme={resolved}
                      brand={ecom.brand || project!.name.replace(' Site', '')}
                      logoIcon={ecom.logoIcon || heroBlock?.content?.logoIcon}
                      projectId={projectId}
                      overrides={ecom.overrides}
                      dbProducts={dbProducts}
                      onAddToCart={(p) => handleAddToCart(p, 'M', 'Black', 1)}
                ...
                {currentPageBlocks.length === 0 ? (
                  ...
                  currentPageBlocks.map((block) => (
                    <section key={block.id}>
                      {block.type === 'products' && !customerSession && !isRealEstate && !isScratchEcommerce ? (
                        ...
                        <BlockMarkup
                          ...
                          wishlist={wishlist}
                          onToggleWishlist={handleToggleWishlist}
                          gymInfo={projectConfig?.businessType === 'gym' ? gymInfo : undefined}
                        />
                      )}
                    </section>
                  ))
