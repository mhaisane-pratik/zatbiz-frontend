...
import WeddingStorefront from '@/components/preview/templates/wedding/WeddingStorefront';
import RestaurantThemeStudio from '@/components/builder/RestaurantThemeStudio';
import GymThemeStudio from '@/components/builder/GymThemeStudio';
import EcommerceThemeStudio from '@/components/builder/EcommerceThemeStudio';
...
export default function BuilderPage({ params }: PageProps) {
  ...
  // Route themed ecommerce stores to the Ecommerce Theme Studio.
  // Variant themes carry a `suffix`; scratch/legacy stores keep the old builder.
  if (
    (projectConfig?.businessType === 'ecommerce' || projectConfig?.businessType === 'shop') &&
    projectConfig?.shopNiche !== 'scratch' &&
    (projectConfig?.themeConfig?.category || projectConfig?.shopNiche)
  ) {
    return (
      <EcommerceThemeStudio
        projectId={projectId}
        project={project}
        showToast={showToast}
      />
  ...
  // Route gym projects to the Gym Theme Studio (landing / login / member portal)
      ...
      return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center text-white">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-xs font-semibold text-slate-400 font-sans">Loading Fitness Customizer Profile...</p>
          </div>
        </div>
      );
    }
  ...
  // Route restaurant projects to the custom Shopify-style Theme Studio
    ...
    if (!restaurantInfo) {
      return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center text-white">
          <div className="flex flex-col items-center gap-3">
            <span className="w-6 h-6 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin" />
            <p className="text-xs font-semibold text-slate-450 font-sans">Loading Restaurant Customizer Profile...</p>
          </div>
        </div>
      );
    }
  ...
  const renderPageCanvasContent = () => {
    if (activePage === 'landing') {
      ...
      // Real estate: render the themed storefront home page (matches the live
      // site & preview) instead of the generic stacked blocks.
      const isRealEstateProject =
        blocks.some((b: any) => (b.id || '').startsWith('realestate-')) ||
      ...
      // Wedding / Event: render the themed storefront home page.
      const isWeddingProject =
        blocks.some((b: any) => (b.id || '').startsWith('wedding-')) ||
