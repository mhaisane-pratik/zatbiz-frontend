...
export default function StorefrontLogin({
  ...
  const illustrationUrl = 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=800&auto=format&fit=crop&q=80';
  ...
  return (
    <main className="min-h-screen bg-slate-50 flex items-stretch text-slate-800 font-sans">
      ...
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(99, 102, 241, 0.4), rgba(15, 23, 42, 0.85)), url('${illustrationUrl}')`
        }}
      >
        ...
        <div className="space-y-4 max-w-md text-left my-auto">
          ...
          <h2 className="text-3xl font-black tracking-tight leading-tight">
            Explore Curated Collections & Deals
          </h2>
          <p className="text-xs text-slate-350 font-medium leading-relaxed">
            Log in to manage your shopping cart, apply promotional discount coupons, checkout securely, and track delivery shipments in real-time.
          </p>
        </div>

        <p className="text-[10px] text-indigo-400 font-black uppercase tracking-wider">
          © 2026 {companyName || 'Retail Shop'}. Shopping Systems.
        </p>
      </div>
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="text-left">
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">
              {isSignUp ? 'Create Customer Profile' : 'Sign In to Store'}
            </h2>
            <p className="text-xs text-slate-400 font-semibold mt-1">
              {isSignUp ? 'Register to apply discounts and place orders.' : 'Sign in to access your shopping dashboard.'}
            </p>
          </div>
            ...
            <div className="rounded-2xl border border-indigo-100 bg-indigo-50/20 p-4 text-left">
              ...
              <div className="mt-2 space-y-1 text-xs text-slate-600">
                <p>
                  ...
                  <span className="bg-white border border-indigo-100 rounded px-1.5 py-0.5 font-mono text-[10px] text-indigo-655 font-bold">admin@gmail.com</span>
                </p>
                <p>
                  <span className="font-bold text-slate-800">Customer Email:</span>{' '}
                  <span className="bg-white border border-indigo-100 rounded px-1.5 py-0.5 font-mono text-[10px] text-indigo-655 font-bold">customer@gmail.com</span>
                </p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full mt-4 rounded-xl py-3 px-4 text-xs font-bold text-white bg-indigo-650 hover:bg-indigo-700 cursor-pointer shadow-md shadow-indigo-500/15 uppercase tracking-wider transition-all"
            >
              {isSignUp ? 'Create Customer Profile' : 'Sign In to Store'}
            </button>
          </form>
          ...
          <div className="flex items-center justify-between text-xs pt-4 border-t border-slate-100">
            <button
              ...
              className="font-bold text-indigo-650 hover:underline bg-transparent border-0 cursor-pointer"
            >
