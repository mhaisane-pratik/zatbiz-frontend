  ...
  useEffect(() => {
    ...
    const fs = `
      ...
      void main() {
          ...
          // Background colors derived from Aura Lumina
          vec3 color1 = vec3(0.427, 0.157, 0.851); // #6d28d9
          vec3 color2 = vec3(0.12, 0.05, 0.25);    // Darker purple
          vec3 accent = vec3(0.98, 0.82, 0.4);    // Gold
          
          float noise = snoise(uv * 2.0 + u_time * 0.1);
          noise += 0.5 * snoise(uv * 4.0 - u_time * 0.05 + mouse * 0.5);
          
          vec3 finalColor = mix(color2, color1, noise * 0.5 + 0.5);
          finalColor += accent * pow(max(0.0, 1.0 - distance(uv, mouse)), 12.0) * 0.5; // Mouse glow
          
          gl_FragColor = vec4(finalColor * 0.15, 1.0); // Keep it subtle as a background
      }
    `;
