...
export default function CustomerDashboard({ projectId, projectConfig, customerSession, onLogout, setActiveView, addToast }: any) {
  ...
  const handleSupportSend = (e: React.FormEvent) => {
    ...
    setSupportMessage('');
    setTimeout(() => {
      setSupportChat(prev => [...prev, { sender: 'bot', text: 'Thank you for reaching out. A support coordinator from the Furniture & Home Decor team has been notified.' }]);
    }, 1000);
  };
  ...
  return (
    <div className={`min-h-screen flex flex-col md:flex-row font-serif ${isLight ? 'bg-slate-50 text-slate-800' : 'bg-slate-955 text-white'}`}>
      {/* Sidebar navigation */}
          ...
          <div className="space-y-6">
            <div className="relative overflow-hidden rounded-[24px] p-8 text-white flex flex-col justify-between min-h-[180px] shadow-xl" style={{ backgroundColor: primaryColor }}>
              ...
              <div className="relative z-10 flex justify-between items-start">
                <div>
                  <h3 className="text-sm font-black uppercase tracking-wider">Furniture & Home Decor VIP VIP CARD</h3>
