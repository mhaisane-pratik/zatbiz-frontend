...
export default function SchoolLogin({
  ...
  const illustrationUrl = 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?w=800&auto=format&fit=crop&q=80';
    ...
    <main className="min-h-screen bg-slate-50 flex items-stretch text-slate-800 font-sans">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xl font-black uppercase tracking-wider">{companyName || 'Campus Hub'}</span>
        </div>
        ...
        <div className="space-y-4 max-w-md text-left my-auto">
          <span className="bg-indigo-500/25 border border-indigo-400/20 px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider">
            Academic Registry & Intranet
          </span>
          <h2 className="text-3xl font-black tracking-tight leading-tight">
            Learn, Grow & Lead Tomorrow
          </h2>
          <p className="text-xs text-slate-350 font-medium leading-relaxed">
            Sign in to check class schedules, access study directories, review tuition billing invoice ledgers, and track grade transcript records.
          </p>
        </div>

        <p className="text-[10px] text-indigo-400 font-black uppercase tracking-wider">
          © 2026 {companyName || 'Campus Hub'}. Student Registry Systems.
        </p>
      </div>
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="text-left">
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">
              {isSignUp ? 'Create Student Account' : 'Sign In to Campus Intranet'}
            </h2>
            <p className="text-xs text-slate-400 font-semibold mt-1">
              {isSignUp ? 'Register below to access student directories.' : 'Sign in to access your course files and class schedule.'}
            </p>
          </div>
            ...
            <div className="rounded-2xl border border-indigo-100 bg-indigo-50/20 p-4 text-left">
              ...
              <div className="mt-2 space-y-1 text-xs text-slate-600">
                ...
                <p>
                  ...
                  <span className="bg-white border border-indigo-100 rounded px-1.5 py-0.5 font-mono text-[10px] text-indigo-650 font-bold">student@gmail.com</span>
                </p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5 text-left">
              ...
              <input
                ...
                name="email"
                required
                defaultValue={isSignUp ? '' : 'student@gmail.com'}
            ...
            <button
              type="submit"
              className="w-full mt-4 rounded-xl py-3 px-4 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 cursor-pointer shadow-md shadow-indigo-500/15 uppercase tracking-wider transition-all"
            >
              {isSignUp ? 'Create Student Account' : 'Verify & Access Intranet'}
            </button>
          </form>
          ...
          <div className="flex items-center justify-between text-xs pt-4 border-t border-slate-100">
            ...
            >
              {isSignUp ? 'Already registered? Sign In' : 'New student? Register Account'}
            </button>
            <Link
