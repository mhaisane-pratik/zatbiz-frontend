...
export default function AiSidekick({ context, activeBlockType, activeBlockId, onExecuteAction }: AiSidekickProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      text: context === 'dashboard' 
        ? "Hello! I am your PrimeZat Sidekick. I can help recommend starting templates, explain features, and write company descriptions. Ask me anything!"
  ...
  // Turn off pulse button animation after user opens it once
  useEffect(() => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/ai/chat', {
        ...
        body: JSON.stringify({
          message: textToSend,
          context: context,
          activeBlockType: activeBlockType || null
        })
      });
      ...
      // Use console.warn instead of console.error to prevent triggering Next.js dev server overlay
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-40 flex items-center gap-2 px-5 py-3.5 bg-gradient-to-r from-indigo-600 via-indigo-650 to-purple-650 text-white font-bold rounded-full shadow-2xl transition hover:scale-105 active:scale-95 cursor-pointer ${
          pulse ? 'animate-bounce' : ''
        }`}
      >
        <span className="text-base select-none">✨</span>
        <span className="text-xs tracking-wide">PrimeZat Sidekick</span>
      </button>
          ...
          {messages.map((msg) => (
            ...
            >
              ...
              >
                ...
                {/* Inline Action Card if applicable */}
...
// Client-side AI fallback simulation in case the backend hasn't compiled or is restarting.
function getSimulatedAiResponse(message: string, context: 'dashboard' | 'builder', activeBlockType?: string | null) {
  ...
  if (msg.includes('coffee') || msg.includes('cafe')) {
    ...
    data.answer = "I am your PrimeZat AI Sidekick! You can chat with me to help generate copy, select layout presets, or perform actions. Try typing: \n- *'write a coffee shop banner'*\n- *'add a pricing block'*\n- *'change theme to sunset'*";
  }
  else {
